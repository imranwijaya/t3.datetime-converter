...
const TimezoneTo: NextPage = () => {
  const [isOpen, setIsOpen] = useState(false);
  const { timezoneTo, setTimezoneTo } = useContext(GlobalContext);
  ...
  const selectOnChangeHandler = (
    ...
    if (newValue && !Array.isArray(newValue)) {
      setTimezoneTo(newValue as ValueProps);
    }
    setIsOpen(false);
  };
  return (
...
const Control: NextPage<ControlProps<ValueProps>> = (props) => {
  return (
    <div className="p-2">
      {/* {Array.isArray(props.options) ? <div className="p-1">{props.options.length} data</div> : null} */}
      <components.Control
        {...props}
